...
Relational Databases are a system in which is used to store data, SQL is the language used to interact with a database system.


- [ ] Why do tables need a `primary key`?
Tables need a PK so that we individually index a record along with having a massive performance boost in our queries when searching for data.
...
We need to create a bridge table so that both tables have access to each others data if need be.
